...
import { subscribeExercisePlan } from "../../Api/ExerciseRecommendation";
...
const ExerciseCard = ({ data }) => {
                    ...
                    { text: "Cancel", style: "cancel" },
                    {
                        ...
                        onPress: (input) => {
                            ...
                            if (weekNumber >= 1 && weekNumber <= 32) {
                                subscribeExercisePlan(data.exerciseplanid, weekNumber);
                            } else {
    ...
    const handleConfirmSubscription =async () => {
        ...
        if (weekNumber >= 1 && weekNumber <= 32) {
            const response=await subscribeExercisePlan(data.exerciseplanid, weekNumber);
            if(response.statusCode==200){
                  Toast.show({
                        type: "success",
                        text1: "Subscribe!",
                        text2: "You have have successfully subscribed.",
                        position: "top",
                        visibilityTime: 2000,
                      });
