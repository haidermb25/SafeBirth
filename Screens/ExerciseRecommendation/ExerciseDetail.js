...
import { GetDetailById } from '../../Api/ExerciseRecommendation'; // Assuming fetchDietDetailById is your API function

const ExerciseDetail = () => {
    const navigation = useNavigation();
    const [selectedTrimester, setSelectedTrimester] = useState('trimester1'); // Default to Trimester 1
    const [ExerciseDetails, setExerciseDetails] = useState([]); // State to store diet details
        ...
        const getExerciseDetails = async () => {
            ...
            try {
                ...
                if (result && result.statusCode === 200) {
                    setExerciseDetails(result.data); // Store fetched data
                } else {
                    setError('No exercise details found');  
                }
            } catch (error) {
        ...
        return (
            <View style={styles.container}>
                <View style={styles.headerContainer}>
                    ...
                    <Text style={styles.header}>Exercise Detail</Text>
                </View>
    ...
    const filterWeeksForTrimester = (trimester) => {
        ...
        return ExerciseDetails.filter(item => item.weeknumber >= startWeek && item.weeknumber <= endWeek);
    };
    ...
    const renderDietDetail = ({ item }) => {
        ...
        const detailsText = details.map((detail, index) => (
            <View key={index} style={styles.detailContainer}>
                <FontAwesome5 name="dumbbell" size={18} color="#34C759" style={styles.mealIcon} />
                <Text style={styles.detailText}>{detail.exercisetype}: {detail.exercisedescription}</Text>
            </View>
        ));
