...
import { getExerciseSubscription } from "../../Api/SubscriptionAPi";

const ExerciseSubscription = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchWeeksData = async () => {
      try {
        const response = await getExerciseSubscription();
        ...
        const groupedWeeks = data.reduce((acc, item) => {
          ...
          if (!acc[item.weeknumber]) {
            acc[item.weeknumber] = {
              ...
              exercises: [], // Store exercises instead of meals
            };
          }

          acc[item.weeknumber].exercises.push(
            `${item.exercisetype || "Unknown"}: ${
              item.exercisedescription || "Not specified"
            }`
          );
          return acc;
        }, {});
  ...
  if (weeksData.length === 0) {
    return (
      <View style={styles.centeredContainer}>
        <Text style={styles.emptyText}>No Exercise plan data available.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerText}>D Subscription</Text>
      </View>

      {/* Scrollable Weeks List */}
      <ScrollView>
        {weeksData.map((item) => {
          const { weeknumber, iscompleted, exercises } = item;
          return (
            <View key={weeknumber} style={styles.weekContainer}>
              <TouchableOpacity
                onPress={() => handleWeekPress(weeknumber)}
